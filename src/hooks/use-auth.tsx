...
const MISSING_PROFILE_TABLE_CODE = 'PGRST205';

let warnedAboutMissingProfilesTable = false;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const fetchProfile = useCallback(async (currentUser: User) => {
    ...
    try {
      ...
      if (error) {
        if (error.code === MISSING_PROFILE_TABLE_CODE) {
          setProfile(profileFromAuthUser(currentUser));

          if (!warnedAboutMissingProfilesTable) {
            warnedAboutMissingProfilesTable = true;
            console.warn(
              '[AuthProvider] public.profiles is missing. Apply the Supabase migrations to enable profile data.'
            );
          }
      ...
      setProfile(data ?? profileFromAuthUser(currentUser));
    } catch (err) {
